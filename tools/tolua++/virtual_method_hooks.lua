...
function parser_hook(s)
	...
	local requires_magic_node
	do      
		local b,e= string.find(s, "^%s*#define%s*LAYER_NODE_FUNC%s*1")
		if b then
			classContainer.curr.flags.requires_layer_node_func= true;
...
function classVirtualClass:supcode()
	...
	if self.flags.parent_object.flags.requires_layer_node_func then
		output("\tLAYER_NODE_FUNC("..self.original_name..");")
	end
...
function VirtualClass()
	...
	if parent.flags.requires_layer_node_func then
  	c:parse("\nstatic "..name.." *node(void);\n");
	end
	pop()
	return c
end
